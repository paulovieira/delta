...
internals.createRenderer = function (){
    ...
    internals.renderer = Function(internals.args, source.join('\n'));
    internals.rendererSrc = internals.cleanSource(internals.renderer);
};

internals.cleanSource = function (renderer){

    const source = renderer.toString()
                    .trim()
                    .split('\n');
    ...
    if (internals.deltaOptions.name){
...
*/
        ...
        // dom is an array containing the top-level elements (the "parent" property is null in those objects)
        const dom = Htmlparser.parseDOM(input, internals.parserOptions);

        internals.source = ["'use strict'"];
